...
namespace FederationTests
{
    ...
    public class WsTrustTestsFixture : SoapTestingServerFixture<Startup>
    {
        ...
        public WsTrustTestsFixture()
        {
            ...
            _handlers.Add(new GodSecurityTokenHandler(), "urn:god", "urn:deity");
            _handlers.Add(new SamlSecurityTokenHandler(), SamlConstants.OasisWssSamlTokenProfile11);
            _handlers.Add(new Saml2SecurityTokenHandler(), Saml2Constants.Saml2TokenProfile11, Saml2Constants.OasisWssSaml2TokenProfile11);
        }
        ...
        //    => builder.AddAspNetCoreHttpsHostFactory(configure);
        ...
        public IWsTrustChannelContract CreateWsTrust13IssuedTokenClient(string subject, string clientTokenType = Saml2TokenType, string appliesTo = "urn:test", string issuer = "test_issuer")
        {
            ...
            var token = CreateSecurityToken(identity, appliesTo, issuer);
            var properties = new Dictionary<string, object>
            {
                { "token", token },
                { "handler", handler }
        ...
        public SecurityToken ConvertSecurityToken(SecurityToken token, Type to)
        {
            ...
            //if (token is System.IdentityModel.Tokens.GenericXmlSecurityToken xmlToken)
            //{
            //    var xml = xmlToken..OuterXml;
        ...
        protected override EndpointAddress CreateEndpointAddress<TChannel>(Uri url, SoapChannelCreationContext context)
            => new EndpointAddress(url, new DnsEndpointIdentity(url.Host));
        ...
        private Binding CreateFederationBinding(SecurityTokenHandler handler)
        {
            ...
            //var binding = new WsTrustHttpBinding(.TransportWithMessageCredential);
        ...
        protected override ChannelFactory<TChannel> CreateChannelFactory<TChannel>(Binding binding, EndpointAddress endpointAddress, SoapChannelCreationContext context)
        {
            ...
            //if (context.Properties.TryGetValue("handler", out var handler))
            //{
            //    var credentials = factory.GetProperty<WsTrustChannelClientCredentials>();
            //    credentials.
        ...
        private SecurityToken CreateSecurityToken(ClaimsIdentity identity, string appliesTo, string issuer, SecurityTokenHandler handler = null)
        {
            var descriptor = new SecurityTokenDescriptor
            {
                Audience = appliesTo,
                Issuer = issuer,
                SigningCredentials = new SigningCredentials(new X509SecurityKey(new X509Certificate2(Convert.FromBase64String(ClientCertificateBase64))), SecurityAlgorithms.RsaSha256Signature, SecurityAlgorithms.Sha256Digest),
                IssuedAt = DateTime.UtcNow,
                NotBefore = DateTime.UtcNow,
                Expires = DateTime.UtcNow.AddHours(1),
                Subject = identity
            };
            return (handler ?? new Saml2SecurityTokenHandler()).CreateToken(descriptor);
        }
    }
}
